...
void Load16bitRegFrom16bitRegPlusValue(uint8_t opcode, Register16bit dest, Register16bit source)
{
	TesterMMU mmu;
	Z80 z80(mmu);

	mmu.Write8bitToMemory(0, 0xDA);
	z80.GetRegisters().Write(source, 0xFFAD);
	z80.Execute(opcode);

	ASSERT_EQ((0xFFAD + static_cast<int8_t>(0xDA)) & 0xFFFF, z80.GetRegisters().Read(dest)) << "Register read unexpected value: "
		<< static_cast<size_t>(z80.GetRegisters().Read(dest));
...
TEST(Z80Test, LoadRegHLFromRegSPPlusValue)
{
	Load16bitRegFrom16bitRegPlusValue(0xF8, Register16bit::HL, Register16bit::SP);
}
...
void Push(uint8_t opcode, Register16bit source)
{
	...
	z80.GetRegisters().Write(source, 0xDA00);
	z80.Execute(opcode);

	ASSERT_EQ(0xDAAD, mmu.Read16bitFromMemory(0x1232)) << "Memory read unexpected value: "
...
void Pop(uint8_t opcode, Register16bit dest)
{
	...
	z80.GetRegisters().Write(Register16bit::SP, 0x1234);
	mmu.Write16bitToMemory(0x1234, 0xDAAD);
	z80.Execute(opcode);
